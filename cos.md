...
Define a differentiated value proposition for each segment, grounded in local market context (regulatory environment, language, preferred vendor relationships)
Draft core messaging framework: tagline, elevator pitch, one-pager copy per segment

#### Go-to-Market Motion

Recommend a sequenced entry approach: which segment to prioritize first, why, and what the initial beachhead looks like
Define the ideal customer profile (ICP) for each segment with firmographic and behavioral criteria
Outline the sales motion: outbound vs. inbound weighting, pilot/POC structure, pricing model hypothesis, and typical deal cycle
Identify 10–15 named target accounts per segment as the initial pipeline seed

#### Execution Roadmap

Deliver a phased 90-day GTM action plan with owners, milestones, and leading indicators

#### Deliverables

Market landscape and competitive analysis report
ICP definitions and named target account lists (per segment)
Messaging framework and one-pager drafts (per segment)
